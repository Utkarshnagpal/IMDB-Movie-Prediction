...
# Saving bins assigned to directors for test set transformation
...
## FINDING IDEAL DEGREES FOR EACH PREDICTOR  ##

### POLYNOMIAL REGRESSION ### 
# To determine which Poly degree is ideal for movieBudget variable 
...
anova(regmB, regmB2, regmB3, regmB4, regmB5) # Since P was not < 0.05 we stop and select Linear so degree = 1 
...
anova(regDuration, regDuration2, regDuration3, regDuration4, regDuration5) # d = 2
...
anova(regNews1, regNews2, regNews3, regNews4, regNews5, regNews6) # d = 5 
...
anova(regst1, regst2, regst3, regst4, regst5, regst6)  # select d = 2 
...
####            ***  Spline regression movieBudget 
# We are going to create 5 knots and divide data into 6 equally spaced  regions 
...
# Let's run the regression for movieBudget 
...
#Let's run the regression for duration
...
####            ***  Spline regression nbNewsArticles
# We are going to create 5 knots and divide data into 6 equally spaced  regions 
...
####            ***  Spline regression movieMeter_IMDBpro
# We are going to create 5 knots and divide data into 6 equally spaced  regions 
...
####            ***  Spline regression movieBudget 
# We are going to create 2 knots and divide data into 6 equally spaced  regions 
...
#removing outliers - refer to appendix for additional information

imdbdf = imdbdf[-c(11,183,383,477,1529,954,1748),]


#saving transformed datset
...
#Removing Heteroskedasticity from selected model
...
## MAKING PREDICTIONS - PROCESSING TEST SET
...
# Joining test set with director ranking bins 
testdf = left_join(testdf, dirbins, by = 'director')
  

#Setting default bin to 5  
...
# Check if certain attributes are converted to categorical ones.
sapply(imdb, is.factor)

# Check for Non-linearity - only numerical predictors
